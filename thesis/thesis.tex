...
Variables over which I have no full control yet measure their influence to the best possible capacity:
\begin{itemize}
  \item allocated network bandwith for cloud instances[TODO - verify],
  \item spikes in resource allocation on shared cloud instances (e.g. Allocated CPU cycles).
  \item general global network conditions (benchmarks are run at the same time of a workweek).
\end{itemize}
