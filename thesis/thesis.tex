...
\cite{GroBuy} N. Grozev, R. Buyya, Multi-Cloud Provisioning and Load Distribution for Three-Tier Applications, ACM Transactions on Autonomous and Adaptive Systems (TAAS), Vol.9 Iss.3, Article No. 13 (October 2014)
...
Cloud computing has become virtually ubiquitous. All biggest internet services are either deployed to a cloud, or run their proprietary cloud systems. Netflix and Spotify (both deployed to Amazon's AWS) are examples of the first approach, with Microsoft(Azure), Google(Google Cloud Platform)\footnote{\url{http://google.com}} and Amazon(Amazon Web Services) being the example of the second. A lot of companies running proprietary clouds also make them available for hosting to the external clients. 
A lot of tools and providers, most popular - google, azure, aws. Described in details in a chapter \ref{Cloud architecture setup} dedicated to this.

Software engineers need to design for the cloud, not only to deploy in it. To facilitate that process, they propose an adaptive dynamic provisioning and autonomous workload redirection algorithms. \cite{GroBuy}
...
Data layer is often a performance bottleneck because of requirements for transactional access and atomicity - it is hard to scale out because of this \cite{GroBuy}.
...
Many solutions and strategies for dealing with database scalability have emerged, including nosql and newsql databases, data replication, caching and database sharding [MMPROJ8]. In Amazon’s environment, Grozev and Buyya suggest using Elasticache service \cite{GroBuy}. We cannot use this, also described in \ref{Cloud architecture setup}.
...
The authors suggest choosing randomly when generating load as to which operation to perform, on what data size etc. They suggest using different random distributions: uniform, zipfian, latest, multinomial[MMPROJ7]. Another distribution is suggested by Grozev and Buyya \cite{GroBuy} - Poisson distribution with a constant mean.
